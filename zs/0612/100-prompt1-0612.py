### this is the udpated prompt 1
# use deepseek, llama distill, the qwen distill is depreciated
...
def extract_entities_only(abstract, doc_id, max_retries=1):
    ...
    example_output = """
                {
                 ...
                 "relationships": [
                   ...
                   ["265 of 500 (53%)", “measurement of”, "Mortality"]
    ...
    prompt = f"""
                    ...
                    Relationships should capture interactions such as  "infects," "hosts," "lives in," "found in," "detected by," "affected by", “causes” or other relevant relations in addition to these examples. If no evidence is present that a given host is infected by a given pathogen like a negative test result, capture the interaction as a negative relation like “does not infect” or “does not host”.
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_llm_response = completion_text
            ...
            for ent in raw_entities:
                if isinstance(ent, list) and len(ent) == 2:
                    entity_text = ent[0]
                    entity_type = ent[1]
                    text_match = re.search(rf"[^.]*\b{re.escape(entity_text)}\b[^.]*\.", abstract, re.IGNORECASE)
                    context = text_match.group(0).strip() if text_match else abstract[:200]
                    entities.append({"text": entity_text, "type": entity_type, "doc_id": doc_id, "context": context})
    ...
    if fallback_ents:
        print(f"Bracket fallback found {len(fallback_ents)} entities for doc {doc_id}.")
        return [{"text": e[0], "type": e[1], "doc_id": doc_id, "context": abstract[:200]} for e in fallback_ents]
...
### prompt instructs the LLM to use only the provided entities—the model may output variants or corrected versions of entity names.
...
def reassign_undefined_types_with_llm(entities):
    """
    Forces ALL undefined entities to be classified by LLM with no exceptions.
    Implements progressive fallback strategies until every entity gets a valid type.
    """
    ...
    for entity in undefined_entities:
        ...
        prompt_versions = [
            # Attempt 1: Detailed expert analysis
            f"""As a world-class biomedical entity classifier, you MUST assign the most specific possible type to this entity.
Entity: "{entity_text}"
Context: "{context}"

Analyze the entity's role and relationships in the context. Respond with ONLY this JSON format:
{{
  "entity": "{entity_text}",
  "type": "Your classification",
  "certainty": "High/Medium/Low",
  "alternatives": ["Possible alternative types"]
}}""",

            # Attempt 2: Simplified forced choice
            f"""CLASSIFY THIS ENTITY - YOU MUST CHOOSE A TYPE:
Entity: "{entity_text}" 
Context: "{context}"
...
{{
  "entity": "{entity_text}",
  "type": "You must provide a classification"
}}""",

            # Attempt 3: Binary choice as last resort
            f"""Is "{entity_text}" more likely to be a (1) BIOLOGICAL entity or (2) NUMERICAL/MEASUREMENT entity?
Context: "{context}"

Respond with JSON:
{{
  "entity": "{entity_text}",
  "type": "Biological_Entity" or "Numerical_Value"
}}"""
        ]
        ...
        for attempt in range(3):
            try:
                response = client.chat.completions.create(
                    messages=[{"role": "user", "content": prompt_versions[attempt]}],
                    model="llama3-8b-8192",
                    temperature=0.3 if attempt > 0 else 0.1,  # More creative in later attempts
                    max_tokens=512,
                    top_p=1.0,
                    seed=42
                )
                ...
                if json_match := re.search(r'\{.*?\}', completion_text, re.DOTALL):
                    json_text = clean_json_string(json_match.group(0))
                    parsed = json.loads(json_text)

                    if parsed.get("type"):
                        entity["type"] = parsed["type"]
                        print(f"Classified '{entity_text}' as '{parsed['type']}' (Attempt {attempt + 1})")
                        break  # Exit retry loop on success

            except Exception as e:
                print(f"Attempt {attempt + 1} failed for '{entity_text}': {e}")

            time.sleep(1)
        else:  # Final fallback if all attempts fail
            entity["type"] = "LLM_Default_Classification"
            print(f"⚠️ Used fallback classification for: {entity_text}")
...
def get_suspicious_types(entities):
    ...
    for e in entities:
        ...
        if (
                type_counts[t] < 3
                or any(word in t for word in
                       ["of", "with", "in", "by", "from", "at", "on", "for", "to", "among", "between", "over",
                        "below", "above"])
                or len(t.split()) > 2
        ):
...
def check_entities_and_relationships(df, all_entities, all_relationships):
    ...
    for idx, row in df.iterrows():
        ...
        # Check if the abs_id is present in the extracted data
...
def main():
    ...
    abstracts = df['title_abs'].iloc[0:101].tolist()  # Exclude header, limit to 10 documents for example
    ...
    for idx, abstract in enumerate(abstracts):
        ...
        # Ensure all relationship endpoints are in the entity list
        doc_entities = augment_entities_with_relationship_endpoints(doc_entities, doc_relationships)

        # Reclassify suspicious types via LLM
        ...
        # Unify conflicting type assignments
    ...
    entities_csv_path = f"entities_{today_str}_no_merge_single_id_llama3-8b.csv"
    relationships_csv_path = f"relationships_{today_str}_no_merge_single_id_llama3-8b.csv"
