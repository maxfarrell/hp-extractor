...
ent_l <- read.csv(files[grepl("entities", files) & grepl("llama3|llama.csv", files)]) %>% distinct
...
rel_l <- read.csv(files[grepl("relationships", files) & grepl("llama3|llama.csv", files)]) %>% distinct
...
_How many entity types is each entity text associated with within each model? 2-3% of entities have >1 type. Ideal is zero?_
...
rel_ll %>% filter(doc_id == (rel_ll %>% filter(!(doc_id %in% c(rel_ll %>% filter(relation_type == "hosts"|relation_type == "infects") %>% pull(doc_id)))) %>% pull(doc_id)))
```
